...
const ProdukTabel = ({navigation}) => {
  ...
  function getDt(mm) {
    const dbRef = database().ref('/produk');
  ...
  function menghapus(nama, id) {
    Alert.alert('PERHATIAN', 'Yakin akan hapus ' + nama, [
      ...
      },
      {
        text: 'OK',
        onPress: () => {
          const hapusRef = database().ref(`/produk/${id}`);
  ...
  const renderItem = ({item}) => {
    return (
      <Item
        data={item}
        onEdit={() => navigation.navigate('ProdukEdit', {produkID: item.id})}
  ...
  return (
    <View style={styles.container}>
      ...
      <FAB
        ...
        onPress={() => navigation.navigate('ProdukAdd')}
      />
      {isLoading && <Loading />}
    </View>
  );
    ...
    <CardList>
      ...
      <View style={[styles.tr, {marginVertical: 10}]}>
        ...
        <View style={{flex: 1}}>
          ...
          <View style={styles.tr}>
            <Text style={styles.tLabel}>Tag</Text>
            <Text selectable style={styles.tValue}>
              {data.tag}
            </Text>
          </View>
          ...
          <View style={styles.tr}>
            <Text style={styles.tLabel}>Stok</Text>
